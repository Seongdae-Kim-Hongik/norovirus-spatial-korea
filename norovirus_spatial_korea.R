...
# ==============================================================================
# Foodborne norovirus spatial analysis — negative binomial, urban/rural 3-group stratified
#   Covariates are pre-specified from theory (separate Total/Urban and Rural candidate sets).
#      Each set passes a single univariable screen (p < 0.20) and a VIF (< 10) check, then the
#      spatiotemporal model is fitted once; effect sizes and directions are reported as estimated.
#
# Transmission chain: person-to-person (contact) + oyster/shellfish (food) + water (wastewater/septic -> rivers) + environment (groundwater)
# Urban/rural class: metro = Sejong + metropolitan cities (not 'gun'); rural = names ending '-gun'; else small city
# Settings: VIF<10 | screening p<0.20 | M6 BYM+RW1+IID (negative binomial) | 2020-2024
# ==============================================================================
rm(list=ls()); gc()
...
if(!dir.exists(DIR_OUT)){ DIR_OUT <- file.path(Sys.getenv("HOME"), "Desktop")
  cat(sprintf("  output folder not writable -> Desktop: %s\n", DIR_OUT))}
...
nb2INLA(nb_obj, file="/tmp/noro.graph"); g_main <- inla.read.graph("/tmp/noro.graph")
...
fn_xlsx<-file.path(DIR_OUT,sprintf("NORO_results_%s.xlsx",TS));saveWorkbook(wb,fn_xlsx,overwrite=TRUE)
cat(sprintf("\n  ★ 엑셀: %s\n",fn_xlsx))
...
cat(sprintf("\n✅ 로그: %s\n",LOG))
sink()
cat(sprintf("\n═══ NORO 사전지정 분석 (전체/도시/농촌) 완료 ═══\n"))
cat(sprintf("★ 모델: %s | 변수선택: 사전지정(이론)\n", FAMILY))
...
if (!exists("res_final")) stop("메인 분석을 먼저 끝까지 실행해야 합니다")
...
#==============================================================================
# ════════════════════════════════════════════════════════════════════════════════
# Paediatric battery — collinearity / VIF diagnostics and dimension reduction (0-4 vs 5-9 child share)
# ════════════════════════════════════════════════════════════════════════════════
...
# Purpose: (1) correlation/distribution of the paediatric covariates, (2) VIF, (3) refit after dropping the 0-4 share
...
# Interpretation: the 0-4 and 5-9 child shares are strongly collinear (r ~ 0.91).
...
# ------------------------------------------------------------------------------
# ★ 실행 전제: 메인 분석 스크립트를 '같은 R 세션'에서 먼저 실행.
...
#==============================================================================
# Counting-unit (case vs outbreak) sensitivity [standalone block] -> Supplementary Table S9
#==============================================================================
# ════════════════════════════════════════════════════════════════════
#  노로바이러스 공간분석 — OUTBREAK(건수) 단위 민감도 분석
#  Reviewer comment response: case (individual patient cases) vs outbreak (event counts) counting-unit sensitivity
#  - 입력: 식중독최종_건수.csv (cases 열 = outbreak 건수)
#  - 비교: 식중독최종.csv (cases 열 = 환자 사례수, 본문 Ch4 = 4,801건)
#  - 기간/지역/모형: Ch4와 동일 (2020–2024, 시군구, NB BYM2)
#  Run in an R / RStudio environment with R-INLA installed.
# ════════════════════════════════════════════════════════════════════
tryCatch({
suppressMessages({
